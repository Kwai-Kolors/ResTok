...
def convert_imagenet_to_wds(output_dir, max_train_samples_per_shard, max_val_samples_per_shard):
    assert not os.path.exists(os.path.join(output_dir, "train", "imagenet-train-000000.tar"))
    assert not os.path.exists(os.path.join(output_dir, "val", "imagenet-val-000000.tar"))

    opat = os.path.join(output_dir, "train", "imagenet-train-%06d.tar")
    ...
    dataset = dataset.cast_column("image", Image(decode=False)) # disable automatic decoding to avoid reading exif
    now = time.time()
    for i, example in enumerate(dataset):
        if i % max_train_samples_per_shard == 0:
            print(i, file=sys.stderr)
        img_bytes, label = example["image"]["bytes"], example["label"]
    ...
    opat = os.path.join(output_dir, "val", "imagenet-val-%06d.tar")
    output = wds.ShardWriter(opat, maxcount=max_val_samples_per_shard)
    dataset = load_dataset("imagenet-1k", streaming=True, split="validation")
    dataset = dataset.cast_column("image", Image(decode=False)) # disable automatic decoding to avoid reading exif
    now = time.time()
    for i, example in enumerate(dataset):
