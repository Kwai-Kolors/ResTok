# Modified from:
#   TiTok: https://github.com/bytedance/1d-tokenizer/blob/main/data/webdataset_reader.py
...
class ImageTransform:
    def __init__(self,
                 random_resized_crop: Union[bool, List[float]] = None,
                 resize_with_crop: Union[float, int] = None,
        ...
        if random_resized_crop is not None:
            if isinstance(random_resized_crop, list):
                train_transform.append(RandomResizedCrop(crop_size, scale=random_resized_crop, interpolation=interpolation, antialias=True))
            else:
                train_transform.append(transforms.Lambda(lambda pil_image: random_crop_arr(pil_image, crop_size)))
        else:
            if resize_with_crop is not None:
                train_transform.append(
                    transforms.Lambda(lambda pil_image: center_crop_arr(pil_image, int(crop_size * resize_with_crop) if isinstance(resize_with_crop, float) else resize_with_crop)),
                )
            elif resize_shorter_edge is not None:
                train_transform.append(
        ...
        eval_transform = []
        if resize_shorter_edge is not None:
            eval_transform.append(transforms.Resize(resize_shorter_edge, interpolation=interpolation, antialias=True))
...
class SimpleImageDataset:
    def __init__(
        ...
        Args:
            ...
            normalize_std: A list of float, the normalization std used to normalize the image tensor.
        """
        transform = ImageTransform(random_resized_crop, resize_with_crop,
...
class PretokenizedWebDataset(SimpleImageDataset):
    def __init__ (
        ...
        random_resized_crop: List[float] = None,
        resize_shorter_edge: int = 256,
        crop_size: int = 256,
        random_crop = True,
        random_flip = True,
        ...
        """
        transform = ImageTransform(random_resized_crop,
            resize_shorter_edge, crop_size, random_crop, random_flip,
            normalize_mean, normalize_std)
