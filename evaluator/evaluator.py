# Modified from:
#   TiTok: https://github.com/bytedance/1d-tokenizer/blob/main/evaluator/evaluator.py
...
class VQGANEvaluator:
    def __init__(
        ...
        self._enable_ms_ssim = enable_ms_ssim
    ...
    def reset_metrics(self):
        ...
        self._lpips_total = torch.zeros(
            1, dtype=torch.float64, device=self._device
        )
        self._ms_ssim_total = torch.zeros(
            1, dtype=torch.float64, device=self._device
        )
        self._ssim_total = torch.zeros(
            1, dtype=torch.float64, device=self._device
        )
        self._psnr_total = torch.zeros(
            1, dtype=torch.float64, device=self._device
        )
    ...
    def update(
        ...
        if self._enable_lpips:
            self._lpips_total += self._lpips_model(fake_images, real_images, normalize=True).sum()

        if self._enable_ms_ssim:
            self._ms_ssim_total += self._ms_ssim(fake_images, real_images, data_range=1.0, size_average=False).sum()

        if self._enable_ssim:
            self._ssim_total += self._ssim(fake_images, real_images, data_range=1.0, size_average=False).sum()
        
        if self._enable_psnr:
            self._psnr_total += (-10 * torch.log10(F.mse_loss(fake_images, real_images, reduction='none').mean(dim=(1,2,3)))).sum()
    ...
    def result(self) -> Mapping[Text, torch.Tensor]:
        ...
        if self._enable_lpips:
            eval_score["LPIPS"] = (self._lpips_total / self._num_examples).item()
        
        if self._enable_ms_ssim:
            eval_score["MS-SSIM"] = (self._ms_ssim_total / self._num_examples).item()

        if self._enable_ssim:
            eval_score["SSIM"] = (self._ssim_total / self._num_examples).item()

        if self._enable_psnr:
            eval_score["PSNR"] = (self._psnr_total / self._num_examples).item()
        ...
        if self._enable_codebook_entropy_measure:
            ...
            eval_score["CodebookEntropy"] = entropy.item()

        return eval_score


class DistributedVQGANEvaluator:
    def __init__(
        self,
        device,
        enable_rfid: bool = True,
        ...
        self._world_size = dist.get_world_size() if is_distributed else 1
    ...
    def _distributed_reduce(self, tensor, op=dist.ReduceOp.SUM):
    ...
    def result(self) -> Mapping[Text, torch.Tensor]:
        ...
        if self._is_distributed:
            # Reduce Tensors
            self._distributed_reduce(self._is_prob_total)
            self._distributed_reduce(self._is_total_kl_d)
            self._distributed_reduce(self._rfid_real_sigma)
            self._distributed_reduce(self._rfid_real_total)
            self._distributed_reduce(self._rfid_fake_sigma)
            self._distributed_reduce(self._rfid_fake_total)
            self._distributed_reduce(self._lpips_total)
            self._distributed_reduce(self._ms_ssim_total)
            self._distributed_reduce(self._ssim_total)
            self._distributed_reduce(self._psnr_total)
            self._distributed_reduce(self._codebook_frequencies)

            # Reduce num_examples
            num_examples_tensor = torch.tensor(self._num_examples, device=self._device, dtype=torch.float64)
            self._distributed_reduce(num_examples_tensor)
            self._num_examples = int(num_examples_tensor.item())
        ...
        # sync codebook usage (set type)
        # method: transform set to vector in 0-1, then all_reduce
        if self._enable_codebook_usage_measure:
            usage_tensor = torch.zeros(self._num_codebook_entries, device=self._device, dtype=torch.float64)
            for idx in self._set_of_codebook_indices:
                usage_tensor[idx] = 1
            self._distributed_reduce(usage_tensor)
            self._set_of_codebook_indices = set(torch.nonzero(usage_tensor).cpu().numpy().flatten())
            usage = float(len(self._set_of_codebook_indices)) / self._num_codebook_entries
            eval_score["CodebookUsage"] = usage
