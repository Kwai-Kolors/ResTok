...
def main(args):
    ...
    config = demo_util.get_config(args.model_config)
    tokenizer_model = demo_util.get_restok_tokenizer(config)
    ...
    for batch in loader:
        ...
        with torch.no_grad():
            if args.quant_way == 'vq':
                indices = tokenizer_model.encode(x_all)[1]["min_encoding_indices"]
        ...
        if args.debug:
            ...
            # qzshape = [B, config.vq_model.token_size, 1, config.vq_model.num_latent_tokens]
            # decode the codes
            with torch.no_grad():
                sample = tokenizer_model.decode_tokens(to_save_img_indices)#, qzshape)
